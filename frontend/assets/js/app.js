...
function editCubicle(id, name, description, hourlyRate, hasBeer) {
    const newName = prompt("Cubicle name", name);
    if (newName === null) return;
    const newDesc = prompt("Description", description || "");
    if (newDesc === null) return;
    const newRateStr = prompt("Hourly rate (₱)", String(hourlyRate));
    ...
    const newHasBeer = confirm("Should this cubicle allow beer? OK = Yes, Cancel = No");

    const payload = {
        name: newName,
        description: newDesc,
        hourly_rate: newRate,
        has_beer: newHasBeer ? 1 : 0,
    };

    apiRequest(`cubicles/${id}`, { method: "PUT", body: payload })
        .then(() => {
            loadCubicles();
        })
        .catch((err) => {
            alert(err.message || "Failed to update cubicle");
        });
}
...
$(function () {
    ...
    // Profile button: open profile modal and prefill
